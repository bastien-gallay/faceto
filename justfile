...
# Run the gate set in CI's order (format → lint → test → js → markdown → book → keyboard →
# firewall → size → workflows → justfile). Not `roadmap-check` — that one needs network. `docs` needs mdbook installed, like `md` needs markdownlint-cli2.
