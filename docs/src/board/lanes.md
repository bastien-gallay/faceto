...
| `external` | External systems | Pink `#F2A0C9` | a system outside your boundary |
...
model records, with external systems and hotspots below.
...
for an event, `C7` for a command, `X2` for an actor (`A` is taken by aggregates), `G` for
external. Hand-authored ids are yours to choose; the convention just makes a log readable at a
glance.
