# Summary

[Introduction](./introduction.md)

# Getting started

- [Installation](./guide/installation.md)
- [Your first board](./guide/first-board.md)
- [The 30-second tour](./guide/tour.md)

# The board

- [Lanes and the colour grammar](./board/lanes.md)
- [Elements and direct editing](./board/elements.md)
- [Edges: connect and disconnect](./board/edges.md)
- [Regions, frontiers, collapse](./board/regions.md)
- [Comments, hotspots and findings](./board/comments.md)
- [Reading the diff overlay](./board/diff.md)
- [Keyboard and gesture reference](./board/keyboard.md)

# Reference

- [CLI](./reference/cli.md)
  - [`render`](./reference/cli/render.md)
  - [`serve`](./reference/cli/serve.md)
  - [`lint`](./reference/cli/lint.md)
  - [`export`](./reference/cli/export.md)
  - [`extract`](./reference/cli/extract.md)
  - [`genesis`](./reference/cli/genesis.md)
  - [`compact`](./reference/cli/compact.md)
- [Board formats](./reference/board-formats.md)
- [The model format](./reference/model-format.md)
- [The event log](./reference/event-log.md)
- [Lint rules](./reference/lint-rules.md)

# Working with agents

- [The context pack](./agents/context-pack.md)
- [The narrate skill](./agents/narrate.md)
- [Variants: propose, review, decide](./agents/variants.md)

# Architecture

- [Pipeline and module map](./architecture/overview.md)
- [Invariants](./architecture/invariants.md)
- [Design decisions](./architecture/decisions.md)

# Project

- [Roadmap](./project/roadmap.md)
- [Contributing](./project/contributing.md)
