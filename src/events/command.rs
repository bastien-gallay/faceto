...
/// caller's "nothing to persist" → `400`) when either endpoint is missing or **blank** (`nonblank`,
/// the same write-seam guard `add`/`rename` use — a raw POST can't mint an edge to a `""` element
/// the client could never produce), or when the two coincide: a self-loop `src == dst` has no
/// rendered path and no place in the grammar, and `replay` has no guard against one, so it is
/// rejected here. Endpoint *existence* is not checked — `comment_to_events` sees only the raw
/// comment, never the `Model`; the client only lets a drop land on a real box, and `replay` is
/// deliberately tolerant of a stray id (cascade-cleaned when its element is removed). A duplicate
/// `connect` is a harmless no-op — `replay` dedups the pair.
...
mod tests {
    ...
    fn edge_comment_guards_reject_self_loops_and_missing_endpoints() {
        ...
        assert!(
            mk(r#"{"kind":"connect","src":"","dst":"E2"}"#).is_empty(),
            "an empty endpoint is a blank id (nonblank guard) — a raw POST can't mint an edge to \"\""
        );
        assert!(
            mk(r#"{"kind":"connect","src":"E1","dst":"  "}"#).is_empty(),
            "a whitespace-only endpoint is blank too"
        );
    }

    #[test]
    fn connect_trims_surrounding_whitespace_on_its_endpoints() {
        let evs = comment_to_events(
