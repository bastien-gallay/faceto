...
- **`F-spike-wardley` #115** is the only remaining `☐ | Now` row in the catalog below — and it is
  ...
  PR #149. Nothing there is left to start, so it does not compete for the crown.)
...
*Discharged 2026-09-05:* the precondition clause above is now history, not a live defect — #121 is
complete on `feat/F-lane-enum` (PR #149) and a foreign-format log is refused. Read the sentence as
