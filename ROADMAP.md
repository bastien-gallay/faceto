...
| F-edge-connect | model · edges | ✅ | ✅ Shipped | Connect / disconnect edges between **existing** elements — before this, edges only entered via the bootstrap `model.json`, so a live board couldn't be wired. The spine already supported it (`EdgeAdded` / `EdgeRemoved` — replay, codec, cascade-on-remove); shipped the gap: a `connect` / `disconnect` mapping in `comment_to_events` (directed `src`/`dst` pair, self-loop rejected) + a client gesture. The gesture is **one toggle**: a select-scoped connect handle (a live-pen dot on the focused box's right border, per #48) drags a wire to another box — blue preview connects, red preview over an already-linked box disconnects. Keyboard parity: `e` arms "connect from", focus a target and `Enter` completes (again to disconnect), Esc cancels. New `src/client/connect.js` module; no `render.rs` change. The C7 "connect / disconnect edge" concept. **P1** · Tracked #53. |
...
| F-extract | CLI · AI loop | ☐ | Next | *(strategic review 2026-07-06 — the missing brick of the conversational AI loop.)* New verb `faceto extract` — **semantic sub-board extraction** from a log into a sibling model/log: by region (`--region K2`), by BFS neighbourhood (`--focus E4 --hops 2` — shares the `adj` BFS with **F-focus-graph** #54: that's the *view*, this is the *file*), by type. Ids preserved so the extract diffs cleanly against the origin (**F-variants**). Pure std, pure `Model → Model`. No competitor offers semantic (vs geometric) extraction. **P1** · Tracked #90. |
