...
**Settled.** The type-safety-at-the-edges arc is closed. The three board concepts that were
strings — a sticky's lane, the board's format tag, and a posted command's `kind` — are closed sets
enforced at their boundaries: `F-format-tag` #121 (PR #148), `F-lane-enum` #117 (PR #149) and
`F-typed-command` #120 (PR #152). `CODING_STANDARDS.md` gained the order of work and the comment
rules (PR #151).

**Open, ranked.** `just sync-roadmap` (PR #157) moved three rows to `Now` from the board, so the
crown is no longer empty — it was filled by the generator, not by a decision, which is worth
knowing before treating the order as intentional.

1. **`F-cli-help` #16 · `F-duplicate` #68 · `F-theme` #71** — the three `☐ | Now` rows in the
   catalog. All three are small and none is a kernel item.
2. **`F-formats-move` #122** — #121 shipped a sealed `Format` with one variant; the dispatch
   `match` it exists for is still unwritten. This is the entry point to the multi-format family,
   and `docs/multi-format-architecture.md`'s step 2 is its design.
3. **`F-pitch-benefits` #102** — open and overdue: the pitch still describes an AI loop as partly
   missing, and it has been runnable since 2026-08-03.
4. **`F-generated-write-contract` #153 · `F-log-repair` #154 · `F-log-read-warning` #156** — the
   three the #120 and #149 reviews raised and deliberately parked. Each has a catalog row.

**One thing to decide, not to do.** `F-spike-wardley` #115 was **closed as completed on
2026-09-05**, but its stated deliverable — a constraints note under `docs/notes/` — is not on disk
(`docs/notes/` holds `f-spike-canvas.md` and `f-inline-add.md` only). So either the probe ran and
its note was never written, or it was dropped and closed. Its row is stuck at `✅ | **Now**`
because closing an issue freezes the row against `sync_roadmap.py` (see `.wrap.md`), and only a
hand edit fixes the Horizon — which nobody should make until that question is answered. Left
untouched on purpose.

Deliberately not ticketed: the CI-gate-set enumerations (the debt paragraph below holds the grep).

**Three things that read as contradictions and are not.**

- `docs/F-container-plan.md`, `docs/notes/f-spike-canvas.md` and `docs/F-inline-edit-brief.typ` name
  `comment_to_events`, which no longer exists. Each now opens with a supersession note: they record
  what was built at their moment and are not instructions to follow.
- `docs/multi-format-architecture.md` still argues for `enum Lane` and ADR-1 at length in the
  pre-ship tense. The bullets carry a strikethrough and a **Done** line; the unstruck prose is the
  argument, kept because `F-docs-multiformat` #127 will publish it.
- `CHANGELOG.md` and the shipped rows below narrate `external` and `comment_to_events` as history.
  They are correct as written — only present-tense inventories have to be true now.

**Still wrong, and left that way.** `examples/faceto-event-sourced.model.json` and its sibling log
both carry a sticky whose text names `comment_to_events` as the source of truth. The log is
append-only, so the honest repair there is an annotate event — but the `model.json` is an authored
source file and could simply be edited. Both were left alone: fixing one and not the other would
make the pair disagree, and the pair is a tracked fixture that several tests render.

**Where deleted things went.** Nothing was deleted this pass. The two renamed source files are
recorded in the rows that renamed them: `src/events/comments.rs` → `src/events/command.rs`, and
...
rows arrived there from a generator, so the ordering is not yet anyone's decision.
Work in a git worktree; the main checkout is shared with other sessions.
```
...
- **`F-spike-wardley` #115** is the only remaining `☐ | Now` row in the catalog below — and it is
  a *timeboxed throwaway probe*, not a build. Its deliverable is a constraints note. (It is no
  longer the only `Now` row: `F-format-tag` #121 sits beside it at `🚧`, complete and in review as
  PR #148. Nothing there is left to start, so it does not compete for the crown.) *Superseded
  2026-09-07: #121 merged in PR #148 — so the "in review" clause is spent too, not only the
  number. #115 is not the only `Now` row either: `just sync-roadmap` moved #16, #68 and #71 there
  from the board, and #115's own row froze at `✅ | **Now**` when the issue closed.*
...
- **The documentation *precondition* is discharged** as of 2026-08-07 (`F-docs-reference` #129,
  ...
  must touch — a starting list, not a proof of completeness. `external` appeared as a canonical
  lane value in far more files than either checklist named; `grep -rl readmodel` was the honest way
  to scope #117, not the checklist. Neither is blocked on documentation any more. *(Both shipped:
  #121 in PR #148, #117 in PR #149 — `external` is a read-only legacy spelling now.)*
...
*Discharged 2026-09-07:* the precondition clause above is now history, not a live defect — #121
merged in PR #148 and a foreign-format log is refused. Read the sentence as
the reason the row was crowned, not as a description of what the code does today; the F-format-tag
row in the catalog carries the four boundaries as built.
...
| F-lane-enum | model · kernel | ✅ | ✅ Shipped | *(multi-format triage 2026-07-26 — 🔑 Enabler; highest-leverage / lowest-cost of the set.)* **`enum Lane`** replaces `kind: String`, making the off-grammar-element panic class **unrepresentable** — the `_ =>` fallbacks in `colour` / `lane_index` vanish and `LANES` / `lane_prefix` become total. Folds in **ADR-1: `external` → `system`** (the pink sticky is any software system, internal *or* external; aligns with C4's `SoftwareSystem`). That rename is a **data migration**, not a string swap — `type:"external"` is persisted in the git-tracked log and both examples, so it maps at the **`upcast`** seam; **keep the prefix letter `G`** (ids `G1…` stay valid). Shares the lane-set seam with **F-lane-flow** #79, **F-floating-hotspots**, **F-es-vocabulary** — touch the lane set once, not four times. Tracked #117. **Shipped 2026-09-07 in PR #149.** The rename cost nothing at rest: `external` is read as `system` and never written back, the prefix stayed `G`, so no id — and therefore no comment and no diff verdict — moved, and no log on disk was rewritten. Two things the row did not anticipate. The panic class did not vanish with the `_ =>` arms: `lane_index` was left looking a lane up in `LANES` and `expect`ing it, which is the same panic one layer down, so it became an exhaustive `match` with a const block cross-checking the array against the ranks (review of #149). And the seam is still not airtight — a variant given a rank and left out of `LANES` compiles and draws no band; closing that needs both lists generated from one declaration, which is its own change. `F-lane-flow` #79 inherits the pairing: reordering the lanes now means editing `LANES` **and** `lane_index`, and the build fails if they disagree. |
...
| F-typed-command | serve · kernel | ✅ | ✅ Shipped | *(multi-format triage 2026-07-26 — structural.)* Parse `POST /comment` into a typed `Command` enum **once**, then match exhaustively, instead of `v.get_str("kind")` against string literals — closing the **double-dispatch** and **silent-drop** review findings. Note the asymmetry: the *log* read path stays tolerant (skip unknown kinds, ignore unknown fields, at the `upcast` seam); the *command* boundary should reject an unknown kind loudly. Prerequisite for a per-format command→event mapping. Tracked #120. |
| F-format-tag | format · kernel | ✅ | ✅ Shipped | *(multi-format triage 2026-07-26 — 🔑 Enabler; smallest item that unblocks the family.)* Give the log an explicit **format tag** — a genesis `BoardFormat` event or a top-level `"format"` in `model.json` — selecting which projector replays it. **Absent → `event-storming`**, the same additive default rule `level` already uses, so every existing log replays unchanged. Carries its docs: `docs/src/reference/event-log.md`, `docs/schema/`, `docs/src/reference/model-format.md`. Tracked #121. **Shipped 2026-09-05 in PR #148.** *(A wrap on 2026-09-07 rewrote this as #149 and had to put it back: the sentence it corrected — "complete and green on `feat/F-lane-enum`, in review as PR #148" — was stale in its first two clauses and right in the third. See the F-lane-enum row for the pattern.)* **This discharges constraint 1 of the canvas spike** (#114): a board this build cannot project is refused at *four* boundaries, not the one the row anticipated — a `BoardFormat` naming an unreadable format, a `"format"` in a `model.json` naming one, a log carrying records but not one recognised event kind, and (`render::comparable`) a `--base` diff whose two sides disagree. Three of those four were not in the original scope: they are the shapes the same silent-empty-board defect takes once you look for it, and the fourth is latent until format #2 exists. Two boundaries deliberately stay lenient — an absent tag reads as `event-storming` (the additive rule above), and a line naming *no* kind at all stays a silent skip, because a typo'd `event` key is a broken line and not evidence of another notation. What did **not** ship: the projector is still one implicit path. `Format` is a sealed enum with a single variant and the dispatch `match` it exists for is F-formats-move's (#122) to write. |
| F-log-repair | CLI · log integrity | ☐ | Later | *(noted in the #149 review, 2026-09-06 — 📐 Table-stakes, M.)* Once **#149** lands there is a class of log record **no build of faceto can ever project** and no verb repairs: a line that is valid JSON with no string `event` key. `compact` refuses it — rightly, since folding rewrites the log from the projection and would delete the line from append-only truth — and the remedy it prints is *repair or remove those lines first*, whose only tool is a text editor pointed at the truth file. That is the operation append-only exists to stop asking for: unguarded, unchecked, and with no `<log>.bak`, because the backup is written by the very command that will not run. The distinction is the whole issue: the `unread` refusal (an unknown kind or lane) has a remedy that **arrives on its own** — compact with a newer faceto — while the `corrupt` one is **permanent**, since no future build will read `{"evnet":…}`. Shapes, not a decision: a read-only `doctor` reporting line numbers; a `repair` quarantining to `<log>.rejected` with a backup; or better documentation, which the two split messages #149 already ship are most of. **Not** `compact --force` — it performs the exact deletion the refusal exists to prevent. Narrow by construction: a truncated write or a conflict left with markers fails at `json::parse` with a line number on every verb, so the realistic sources are a misspelled key, a merge that took a whole line from the wrong side, or a foreign generator. **Filed here, out of family:** the rows around it are the `multi-format triage 2026-07-26` cluster and this one is not — it sits beside `F-format-tag` because that is the work whose refusal created the gap, and the two should be read together rather than because it shares their seam. Tracked #154. |
| F-log-read-warning | CLI · diagnostics | ☐ | Next | *(raised in the #149 review, 2026-09-07 — 📐 Table-stakes, S.)* **Filed here, out of family:** it sits beside `F-log-repair` because both are about what a log read does not say, not because it shares the multi-format seam around them. The same defect is announced on one read path and swallowed on the other. `model::load` **warns** — *"N element(s) not readable as a sticky"* — on the principle #117 states: dropping is right, doing it in silence is not. The log path computes the identical count and throws it away: `parse_log_full` returns `LogRead { events, unread, corrupt }` and `events::load` keeps only `events`, so only `compact` ever consults it — because only `compact` would destroy the records. A log holding a lane this build cannot name renders `1 elements` out of two, exit 0, without a word, while the byte-identical `model.json` warns. The CLI half is a transcription: `render` / `lint` / `export` / `extract` all funnel through one `load_source`. The **`serve` half is the actual decision**, and it is why this is not a missing `eprintln!`: `Ctx::current` rebuilds the projection only when the log's content hash changes, so "warn on read" fires at startup *and again after every appended event* — a flood on a different clock, not a diagnostic. Warning once at startup, or surfacing it in the sidebar beside the `kind:"lint"` entries where someone is actually looking, are different contracts from `model::load`'s, and picking one is the work. Tracked #156. |
...
| F-generated-write-contract | docs · tooling | ☐ | Next | *(fell out of #120 / #152, 2026-09-06 — Chore, M.)* The `POST /comment` **write contract** — which `kind`s exist, what each requires, what each appends — is written out by hand in **three** places: `src/events/command.rs`, `docs/src/reference/event-log.md` § *The write path*, and the narrate skill's table. Only the first is checked by a compiler; the other two drift **silently**, and an agent simply does not learn that an action exists — which is how the shipped `connect` / `disconnect` kinds went unlisted. `.wrap.md` set the trigger itself: *if the `comment_to_events` seam moves again, generate the table.* #120 moved it (`parse_command` + `fold_to_events`) and supplied the missing precondition — the kinds are a **closed enum** now, so the table finally has something to generate *from*. **Generate, do not check**: the `keyboard sheet` job is the precedent and shows the limit, since it compares only the `<kbd>` sets, while none of the three known drifts here (`resolve`'s optional `text`, the `{"ok":false}` rejection shape, `connect` not checking its endpoints) was a missing or extra *name* — two were wrong *description* columns and the third was the prose above the table, per the `F-docs-reference` row below. A name-only check sees none of them. Build- or CI-time tool, never linked into the binary — `scripts/check_keyboard_sheet.py` is the shape. Retires the narrate-skill row of `AGENTS.md`'s routing table and one entry of `.wrap.md`'s hand-maintained-duplications list. Tracked #153. |
...
| F-cli-help | CLI · ergonomics | ☐ | **Now** | `--help` / `-h` per subcommand (`faceto render --help` currently treats `--help` as a file path); **plus `faceto <file>` defaulting to `serve`** — the primary action — while `render` / `genesis` / `compact` stay explicit. Small CLI-dispatch ergonomics in `main.rs`. Field feedback #13 §1 + dogfood #12 (reconciled). Tracked #16. |
...
Wiring a direct rename surfaces a latent defect: the `rename` arm of the command boundary
(`comment_to_events` at the time, `parse_command` / `fold_to_events` since #120) and `replay`
accept a **blank label**, persisting a never-renumbered empty box — the exact
...
**F-lane-flow — merge without breaking the grammar.** A merged lane (aggregate+system,
