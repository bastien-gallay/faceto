...
- **[Project #2](https://github.com/users/bastien-gallay/projects/2)** — canonical for **priority
  and order**, held as sortable single-select fields: `Horizon` (Now/Next/Later/Parked/Shipped),
...
1. **`F-extract` #90** — ⚡ · M — semantic sub-board extraction (standalone JTBD *and* proves the
   differentiation). **The only one still open.**
...
| F-scene-ir | render · kernel | ☐ | Next | *(multi-format triage 2026-07-26 — 🔑 Enabler; the decision itself predates it.)* **Data-Scene IR**: `kernel/scene.rs` holds geometric primitives (`Rect`/`Line`/`Text`/`Circle`/`Path` + a **nested** `Group`) and one `render_scene` serializer; ES render becomes `Model -> Scene`. Drivers, strongest last: serializer written once → **diff overlay composes two Scenes** (format-agnostic `diff_meta`) → **Scene-level tests** replace SVG-substring assertions → **the client can split at all**. The IR stays **geometric, never semantic** (`Sticky`/`Region` are ES-only). Correct with one format — does not wait for format #2. Tracked #116. |
...
| F-board-vs-diff | model · kernel | ☐ | Next | *(multi-format triage 2026-07-26 — structural.)* Split the board type from the diff/overlay type: `Model` carries `diff` / `was` / `status` optionals today, so one product type encodes both "the board" and "a diff of two boards". This is the **same boundary the Scene IR wants** — the diff is a render concern, not a domain fact. Cheap right after F-scene-ir; otherwise every new format inherits the confusion. Tracked #119. |
...
| F-extract | CLI · AI loop | ☐ | **Now** | *(strategic review 2026-07-06 — the missing brick of the conversational AI loop.)* New verb `faceto extract` — **semantic sub-board extraction** from a log into a sibling model/log: by region (`--region K2`), by BFS neighbourhood (`--focus E4 --hops 2` — shares the `adj` BFS with **F-focus-graph** #54: that's the *view*, this is the *file*), by type. Ids preserved so the extract diffs cleanly against the origin (**F-variants**). Pure std, pure `Model → Model`. No competitor offers semantic (vs geometric) extraction. Tracked #90. |
