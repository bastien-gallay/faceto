...
1. **`F-extract` #90** — ⚡ · M — semantic sub-board extraction (standalone JTBD *and* proves the
   differentiation).
2. **`F-variants` #91** — ⚡ · S — cross-log diff overlay; closes the loop, cheap (`diff_models` +
   the SVG overlay already exist).
3. **`F-context-pack` #97** — ⚡ · S — the H2 wedge (typed-model-as-spec), rides the `export` seam;
   ship with **`F-pitch-benefits` #102** (📐 · S, same story).
...
| F-variants | render · AI loop | ☐ | **Now** | *(strategic review 2026-07-06 — the "show me with solution B" step.)* **Cross-log diff**: `render a.jsonl --base b.jsonl` and/or `serve --variant sibling.jsonl` — an agent builds a variant in a sibling log, the user sees the added/removed/moved overlay. `diff_models` + the SVG overlay exist; the delta is only the cross-file entry point. With F-extract: extract → variant → diff = the full "what if" loop. Step toward the PR-diff gap (**F-living-doc** #98). Tracked #91. |
