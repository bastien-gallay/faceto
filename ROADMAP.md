...
| F-model-export | CLI · source format | ☐ | Next | **Consolidate to one truth + one source.** (1) **Cut** the legacy `comments.jsonl` import: remove `from_comments` / `comment_to_events` + the genesis sibling-fold + tests (any stray inbox is handled outside faceto). (2) **Reframe `model.json` as the *source / authoring* format** — kill the "legacy `model.json`" wording in code comments (`events/log.rs`) and docs; it is the human/LLM-authored input, not deprecated. (3) New **`export`** verb (`event-log.jsonl` → `model.json`), the inverse of `genesis`/`from_model`; needs a `Model → model.json` serializer (only SVG/HTML come out today) plus a genesis→export round-trip test. Name chosen over "freeze" (implies immutable — the output re-enters the edit loop). (4) Client header: replace the (log-mode no-op) **Export comments** with one **`Export ▾`** menu → **History** (raw log) + **Model** (current state → `model.json`), preserving an offline escape for localStorage-queued gestures. Reconciles with **F-commit-flow** and **F-comment-lifecycle** (same comment-representation seam). **On implementation, update the README**: flip the lifecycle diagram's `export` arrow from *planned* to shipped and revise the source-format prose. Decided 2026-07-04; slice (1) — the `comments.jsonl` cut — in progress. |
| F-dep-policy | build · CI | ☐ | Next | **Reshape "zero deps, ever" → zero *runtime* deps + a size budget.** The old rule counts every `Cargo.lock` package — wrong scope: it forbids test-only crates and measures *count*, not the real fear, *size*. New policy: **core is pure-std at runtime** — CI enforces the **normal** graph only (`cargo tree -e normal --prefix none \| sort -u` → just `faceto`); **`[dev-dependencies]` are free** (unblocks a property-testing crate, never built by `cargo install`); a **binary-size budget** job guards growth (anchor: release binary **905K** → ceiling ~2 MB, tunable). **Local-*first*, not local-only** — collaboration stays allowed. Rewords `Cargo.toml` / `CLAUDE.md` / README + the `runtime deps: 0` badge. Parked seam: a future `serve`/remote/collab layer may split into a pluggable tool with its own looser rules. Brainstorm: `brainstorm/20260704-zero-dep-constraint-reshape.md`. |
