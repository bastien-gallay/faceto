...
inline in each row. Issues not yet minted (gh auth was down at triage time).
...
| F-output-naming | CLI · output | ☐ | Next | Derive `board.svg` / `index.html` names from the **model basename** so sibling boards in one directory don't clobber each other. Small correctness win. Field feedback #13 §1. **Absorbs two F-auto-genesis review carry-overs (PR #23):** (#4) the served log name is hard-coded `event-log.jsonl` regardless of source basename, so `faceto serve b.json` beside a log genesis'd from `a.json` silently serves the *wrong* board — basename-derived log names fix this by construction; (#3) `serve`/`genesis` accept any JSON as a model (`model::from_json` is lenient) and a mis-suffixed `model.jsonl` replays to an empty board — add a guard that warns when a source replays to **0 events / 0 elements**. Both were deferred here rather than fixed piecemeal on the F-auto-genesis branch. |
